...
impl RedisRateLimiter {
    ...
    /// Get client IP from HttpRequest
    fn get_client_ip_from_http(req: &HttpRequest) -> String {
        req.connection_info()
            .peer_addr()
            .map(|s| s.to_string())
    ...
    /// Get client IP from ServiceRequest
    ...
    /// Check rate limit using Redis for HttpRequest
    ...
    pub async fn check_limit(
        ...
        let client_ip = Self::get_client_ip_from_http(req);
    ...
    /// Check rate limit using Redis for ServiceRequest (for middleware)
    /// Returns: (is_allowed, remaining_attempts, reset_seconds)
    pub async fn check_limit_service(
        &self,
        req: &ServiceRequest,
        endpoint: &str,
        max_attempts: u32,
