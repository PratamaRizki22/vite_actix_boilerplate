...
const HomePage = () => {
  ...
  const handleViewComments = async (postId, postTitle, postUsername, postUserId) => {
    try {
          ...
          {loading ? (
            ...
            <div className="space-y-4">
              {posts.map((post) => (
                <div
                  key={post.id}
                  className={`border border-black p-6 transition-all ${commentsPanelData && commentsPanelData.postId === post.id
                ...
                >
                  ...
                  <div className="flex gap-2 mb-4">
                    ...
                    <button
                      onClick={() => handleViewComments(post.id, post.title, post.username, post.user_id)}
                    ...
                    {(post.user_id === user?.id || user?.role === 'admin') && (
                      <button
                        ...
                        className="bg-red-600 hover:bg-red-800 text-white border border-red-600 font-bold py-1 px-3 text-sm transition"
                      >
                        Delete
                      </button>
                    )}
                  ...
                  commentsPanelData.comments.map(comment => (
                    <div key={comment.id} className="mb-3 pb-3 border-b border-gray-300 last:border-b-0 group">
                      <div className="flex items-start justify-between gap-2">
                        ...
                        {/* Show delete button if: user is comment owner OR user is admin OR user is post owner */}
                        {(comment.user_id === user?.id || user?.role === 'admin' || commentsPanelData.postUserId === user?.id) && (
                          <button
